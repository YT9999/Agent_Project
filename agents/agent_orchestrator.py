...
class GeneralAgent(BaseAgent):
    ...
    system_prompt = (
        "你是 NovaRelay 智能客服。友好、简洁地回答用户问题。"
    ...
    async def compose(self, req: Request, responses: List[AgentResponse]) -> str:
        ...
        try:
            response = await self._client.messages.create(
                model=self._model,
                max_tokens=_env_int("NOVARELAY_COMPOSER_MAX_TOKENS", 1000),
                temperature=_env_float("NOVARELAY_COMPOSER_TEMPERATURE", 0.1),
...
class AgentOrchestrator:
    ...
    def __init__(
        ...
        self._shared_tools: Dict[str, AgentToolSpec] = {}
        self._recent_tool_traces = deque(maxlen=_env_int("NOVARELAY_TOOL_TRACE_MAX", 200))
    ...
    def _make_agent(
        ...
        env_name = f"NOVARELAY_{agent_cls.agent_type.value.upper()}_MODEL"
