...
本文档展示 NovaRelay 的核心架构图。图片均已生成到 `wiki/assets/architecture/`，可直接在 wiki、README 或项目展示材料中引用。

当前架构重点体现：Docker Compose 一键部署、结构化多 Agent 路由、RAG 知识库、三级记忆、动态 Skills、监控和端到端评测。

## 1. 总体架构

![NovaRelay 总体架构](assets/architecture/01-overall-architecture.svg)

说明：

- 用户请求先进入 Nginx，再转发到 NovaRelay 主服务。
...
![NovaRelay /chat 主链路架构](assets/architecture/02-chat-flow.svg)
...
![NovaRelay 多 Agent 与 Skills 注入关系](assets/architecture/03-agent-skills.svg)
...
![NovaRelay 数据与存储架构](assets/architecture/04-data-storage.svg)
...
![NovaRelay 监控与评测闭环](assets/architecture/05-monitor-eval.svg)
...
![NovaRelay Docker Compose 一键部署结构](assets/architecture/06-deployment.svg)
