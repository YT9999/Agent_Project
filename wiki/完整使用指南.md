...
本文档说明 NovaRelay 的部署、启动、API 调用、知识库使用、ChromaDB 数据查看、监控评测和常见排障。

重要说明：NovaRelay 当前已经支持可直接调用的端到端评测能力。启动 HTTP 服务后，可以通过 Swagger 或 `curl` 调用 `POST /eval/run`，自动评测意图识别准确率、端到端 Agent 回复质量、LLM-as-Judge 四维评分、回归检测和优化建议。该能力不是文档设计稿，而是由 `evaluation/evaluator.py` 和 `api/main.py` 中的 `/eval/run` 接口真实实现。

NovaRelay 是一个企业级智能客服系统，核心链路为：

```text
用户请求
  -> FastAPI /chat
  -> MemoryManager 读取 Redis 工作记忆 + ChromaDB 情景记忆 + 用户画像
  -> IntentRecognizer 识别细粒度意图 + 意图组 + 结构化实体
  -> 按意图决定是否触发 RAG 知识库检索
  -> AgentOrchestrator 路由到 General/Technical/Billing Agent
  -> SkillManager 按消息关键词和 Agent 类型匹配 Skills
  -> Agent 基于记忆 + 知识库 + 结构化实体 + Skills 调用 LLM 生成回复
...
评测请求
  ...
  -> EndToEndEvaluator 调用 Orchestrator 生成真实 Agent 回复
  -> LLMJudge 从相关性、准确性、完整性、有用性四个维度评分
...
├── core/skill_loader.py           # Skills 加载、解析、匹配和 prompt 注入
...
├── skills/                        # 可热加载的客服 Skills 文档
│   ├── general_customer_service/  # 通用客服接待规范
│   ├── technical_support/         # 技术支持处理规范
│   └── billing_support/           # 账单退款处理规范
...
```env
NOVARELAY_SKILLS_DIR=./skills
NOVARELAY_SKILLS_MAX_PROMPT_CHARS=5000
```

### 2.3 全栈部署和 run 开发模式的区别

NovaRelay 常用两种 Docker 启动方式：`docker compose up` 全栈部署，以及 `docker run` 开发模式。两者最大的区别是：**全栈部署会同时启动应用和依赖服务；run 开发模式通常只手动运行一个应用容器，依赖服务需要提前启动**。

| 对比项 | Docker Compose 全栈部署 | Docker run 开发模式 |
|--------|--------------------------|----------------------|
| 启动命令 | `docker compose up -d --build` | `docker run ... novarelay ...` |
| 启动内容 | NovaRelay、Redis、ChromaDB、Prometheus、Nginx | 只启动你指定的单个容器 |
| Redis/ChromaDB | 自动启动并加入同一网络 | 必须先执行 `docker compose up -d redis chromadb` |
| 容器网络 | Compose 自动创建并管理 | 需要手动指定 `--network novarelay_novarelay-network` |
...
- 如果只是跑 CLI，最省心的方式是 `docker compose run --rm novarelay python api/main.py --cli`，它会自动使用 Compose 网络。
...
```bash
docker compose logs -f novarelay
```

看到 NovaRelay 启动日志并且健康检查通过后，服务可用。

启动后的端口：

| 服务 | 容器名 | 宿主机端口 | 容器内端口 | 用途 |
|------|--------|------------|------------|------|
| NovaRelay API | `novarelay-app` | `8000` | `8000` | 主 API 服务 |
| Nginx | `novarelay-nginx` | `80` | `80` | 反向代理 |
| ChromaDB | `novarelay-chromadb` | `8001` | `8000` | 向量数据库 |
| Redis | `novarelay-redis` | `6379` | `6379` | 工作记忆 |
| Prometheus | `novarelay-prometheus` | `9090` | `9090` | 监控数据 |
...
```bash
docker compose build --no-cache novarelay
```
...
```bash
docker run -it --rm \
  --network novarelay_novarelay-network \
  -p 8000:8000 \
  -e ANTHROPIC_BASE_URL="https://api.deepseek.com/anthropic" \
  -e ANTHROPIC_API_KEY="your_key" \
  -e ANTHROPIC_MODEL="deepseek-v4-pro" \
  -e REDIS_URL="redis://:novarelay123@redis:6379/0" \
...
NovaRelay 基于 FastAPI 构建，启动 HTTP 服务后可以直接在浏览器访问 Swagger UI 调用接口。
...
NovaRelay 的知识库由 `mcp/knowledge_base.py` 管理，底层使用 ChromaDB collection：
...
NovaRelay 使用了三个 ChromaDB collection：
...
```bash
docker exec -it novarelay-chromadb curl http://localhost:8000/api/v1/heartbeat
```
...
```bash
docker exec -it novarelay-app bash
```
...
docker volume inspect novarelay_chromadb-data
```

查看容器内数据目录：

```bash
docker exec -it novarelay-chromadb sh
...
```bash
docker compose down
docker volume rm novarelay_chromadb-data
docker compose up -d --build
```
...
```bash
docker exec -it novarelay-redis redis-cli -a novarelay123
```
...
```bash
docker compose restart novarelay
```
...
```bash
docker exec -it novarelay-redis redis-cli -a novarelay123 ping
```
...
2. 等待几秒
3. 查看 `docker compose logs -f novarelay` 是否出现 `用户画像已更新`
...
# 6. 检索
curl -X POST "http://localhost:8000/search?query=NovaRelay如何接入API&top_k=3"
